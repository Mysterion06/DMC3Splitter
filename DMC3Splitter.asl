...
state("dmc3")
{
    ...
    int level:              0xC8F250;  					                                // The number of the level we are on
    ...
    int isLoadingHD:        0xC90440;                                                   // Something to do with the "loading" on HD, possibly the sprites
    int menuHD:             0xCF2680, 0xD8, 0x110, 0xC0, 0x2F8, 0xD0;                   // When in the main menu
    int gold:               0xC8F340;                                                   // Value of the owned gold orbs
    int ngPlusReset:        0xC9DE6C;
    int bossHealthHD:       0xC90FC0, 0x160, 0x1F0, 0x348, 0x50, 0x260, 0x300, 0xC0;    // Vergil's health when using default costume/default with infinite dt
    int bossHealthBarHD:    0xD544D0, 0x2A8, 0x2F8, 0x100, 0x38, 0x424;                 // Something to do with the boss health bar, not entirely sure, but it's consistent
}
...
state("dmc3se")
{
    ...
    int bossHealth:         0x205CFC8, 0x658;
...
init
{
    ...
    // current.roomID, old.roomID, current.level, vars.split
    vars.doorSplit = new List<Tuple<int, int, int, int>>{
        ...
        Tuple.Create(124, 123, 6, 28),
        Tuple.Create(125, 123, 6, 28),
        Tuple.Create(126, 123, 6, 28),
        Tuple.Create(124, 123, 6, 29),
        Tuple.Create(125, 123, 6, 29),
        Tuple.Create(126, 123, 6, 29),
        Tuple.Create(124, 123, 6, 30),
        Tuple.Create(125, 123, 6, 30),
        Tuple.Create(126, 123, 6, 30),
        ...
        Tuple.Create(403, 412, 18, 162),
        Tuple.Create(403, 414, 18, 162),
        Tuple.Create(403, 416, 18, 162),
        Tuple.Create(403, 412, 18, 163),
        Tuple.Create(403, 414, 18, 163),
        Tuple.Create(403, 416, 18, 163),
        Tuple.Create(403, 412, 18, 164),
        Tuple.Create(403, 414, 18, 164),
        Tuple.Create(403, 416, 18, 164),
        Tuple.Create(403, 412, 18, 165),
        Tuple.Create(403, 414, 18, 165),
        Tuple.Create(403, 416, 18, 165),
        Tuple.Create(403, 412, 18, 166),
        Tuple.Create(403, 414, 18, 166),
        Tuple.Create(403, 416, 18, 166),
...
start
{
    ...
    if(settings["HD"]){
        ...
        if(current.NGStartHD == 5 && old.NGStartHD == 4 && current.level == 1 && current.falseStartHD != 1 && current.mainMenuHD != 0){
            vars.bulletSplit = 0;
            vars.split = 0;
            return true;
        }
...
split
{
    ...
    if(settings["HD"]){
        ...
        if(current.roomID == 411 && current.level == 20 && current.bossHealthBarHD > 10000)
            return current.bossHealthHD == 0 && old.bossHealthHD > 0;
    }
    ...
    if((settings["SEDS"] || settings["DS"]) && vars.doorSplit.Contains(Tuple.Create(current.roomID, old.roomID, current.level, vars.split))){
        vars.split++;
        return true;
    }
}
...
isLoading
{
    ...
    // Removes loads when entering doors and the pause menu
    if(settings["HD"] && settings["LoadRemover"]) return current.LS == 1;
    if(settings["SE"] && settings["LoadRemover"]) return current.gameState == 1;
}
